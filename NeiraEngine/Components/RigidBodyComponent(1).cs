    ...
    class RigidBodyComponent : Component
    {
        ...
        public override void Start()
        {
            ...
            if (meshComponent != null)
            {
                ...
                {
                    CollisionShape shape = PhysicsLoader.loadConvexHull(mesh.mesh, EngineHelper.otk2bullet(mesh.transformation.ExtractScale()));

                    Matrix transformation = EngineHelper.otk2bullet(mesh.transformation.ClearScale());

                    RigidBodyObject rigid_body_object = PhysicsHelper.createLocalRigidBody(
                        worldObject.id + "_" + mesh.id.Replace('.', '_'),
                        EngineDriver.currentDriver._physics_driver.physics_world,
                        dynamic,
                        kinematic,
                        mass,
                        restitution,
