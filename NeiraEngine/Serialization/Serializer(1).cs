...
namespace NeiraEngine.Serialization
{
    public class Serializer
    {

        public string path_save_data { get; }

        public Serializer(string base_data_path)
        {
            path_save_data = base_data_path;
        }
        ...
        public void Save(object instanced_class, string fileName)
        {
            ...
            try
            {
                ...
                stream = new FileStream(path_save_data + fileName, FileMode.Create, FileAccess.Write, FileShare.None);
        ...
        public object Load(string fileName)
        {
            ...
            {
                IFormatter formatter = new BinaryFormatter();
                stream = new FileStream(path_save_data + fileName, FileMode.Open, FileAccess.Read, FileShare.None);
            ...
            {
                Debug.Debug.logError("Cannot Load Data (" + fileName + ")", e.Message);
                return null;
            }
            finally
            {
        ...
        public bool Save(Type static_class, string filename)
        {
            try
            {
                ...
                Stream f = File.Open(path_save_data + filename, FileMode.Create);
        ...
        public bool Load(Type static_class, string filename)
        {
            try
            {
                FieldInfo[] fields = static_class.GetFields(BindingFlags.Static | BindingFlags.Public);
                object[,] a;
                Stream f = File.Open(path_save_data + filename, FileMode.Open);
                BinaryFormatter formatter = new BinaryFormatter();
            ...
            catch (Exception ex)
            {
                if (ex.ToString().Contains("FileNotFound"))
                {
                    ...
                    Save(static_class, path_save_data + filename);
                    return true;
                }
                else
                {
