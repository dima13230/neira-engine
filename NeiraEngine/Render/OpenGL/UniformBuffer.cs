...
namespace NeiraEngine.Render.OpenGL
{
    ...
    public class UniformBuffer
    {
        ...
        private void load(BufferStorageFlags storage_flags, int index, int size)
        {
            ...
            GL.BindBuffer(gl.BufferTarget.UniformBuffer, _buffer_id);
            GL.BufferStorage(gl.BufferTarget.UniformBuffer, size, (IntPtr)null, (OpenTK.Graphics.OpenGL.BufferStorageFlags)storage_flags);
            GL.BindBuffer(gl.BufferTarget.UniformBuffer, 0);

            //Bind uniform buffer to binding index since the block size is set and ubo is created
            GL.BindBufferBase(gl.BufferRangeTarget.UniformBuffer, index, _buffer_id);
        }
