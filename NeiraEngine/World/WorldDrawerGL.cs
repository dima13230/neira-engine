...
{
    internal static class WorldDrawerGL
    {

        private static void trySetBindlessTextureUnit(Program program, Image image, string uTextureUnit, string uEnableTexture, int texture_unit)
        {
            if (image != null)
            {
        ...
        private static void draw(Mesh mesh, BeginMode begin_mode, string mesh_category)
        {
            try
            {
                ...
                GL.BindBuffer(gl.BufferTarget.ElementArrayBuffer, mesh.ibo);
                GL.DrawElements((gl.BeginMode)begin_mode, mesh.index_data.Length, gl.DrawElementsType.UnsignedInt, 0);
        ...
        public static void drawMeshes(BeginMode begin_mode, List<UniqueMesh> meshes, Program program, Matrix4 transformation, float animation_time, int draw_mode)
        {
            foreach (UniqueMesh unique_mesh in meshes)
            {
                ...
                if (unique_mesh.physical)
                {
                    ...
                    {
                        BulletSharp.Math.Matrix temp_transform = temp_mat.ClearScale();
                    ...
                    temp_mat = Matrix4.CreateScale(temp_mat.ExtractScale()) * EngineHelper.bullet2neira(unique_mesh.physics_object.body.MotionState.WorldTransform);
                }

                //------------------------------------------------------
                // World Matrix
                //------------------------------------------------------
                OGL.Uniform(program.getUniform(RenderHelper.uModel), false, temp_mat);

                // Handle previous frame's world matrix
                OGL.Uniform(program.getUniform(RenderHelper.uModel_Previous), false, temp_mat_previous);
                ...
                OGL.Uniform(program.getUniform(RenderHelper.uModel_Normal), false, temp_mat);
        ...
        public static void drawLights(BeginMode begin_mode, List<Light> lights, Program program, Matrix4 transformation, float animation_time, bool display_light_bounds)
        {
            ...
            foreach (Light light in lights)
            {
                ...
                if (display_light_bounds)
                {
                    ...
                    GL.PolygonMode(gl.MaterialFace.FrontAndBack, gl.PolygonMode.Line);
                    ...
                    GL.PolygonMode(gl.MaterialFace.FrontAndBack, gl.PolygonMode.Fill);
                }
            }
        }
