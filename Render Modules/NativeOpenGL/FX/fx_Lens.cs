...
{
    public class fx_Lens : RenderEffect
    {
        // Properties
        ...
        public fx_Lens(string resource_folder_name, Resolution full_resolution)
        ...
        {
            _pBlend = ProgramLoader.createProgram_PostProcessing(new ShaderFile[]
            {
            ...
            _pBrightSpots = ProgramLoader.createProgram_PostProcessing(new ShaderFile[]
            {
                new ShaderFile(ShaderType.FragmentShader, _path_glsl_effect + "lens_BrightSpots.frag", null)
            });
            _pBrightSpots.enable_Samplers(1);

            _pFlare = ProgramLoader.createProgram_PostProcessing(new ShaderFile[]
            {
        ...
        {
            // Load Lens Images
            _iLensColor = StaticImageLoader.createImage(_path_static_textures + "lens_Color.png", TextureTarget.Texture1D, TextureWrapMode.ClampToEdge);
            _iLensDirt = StaticImageLoader.createImage(_path_static_textures + "lens_Dirt-5.jpg", TextureTarget.Texture2D, TextureWrapMode.ClampToEdge);
            _iLensStar = StaticImageLoader.createImage(_path_static_textures + "lens_Star.png", TextureTarget.Texture2D, TextureWrapMode.ClampToEdge);
        ...
        private void getBrightSpots(fx_Quad quad, Texture scene_texture)
        {
            ...
            OGL.Viewport(0, 0, _tBloom.width, _tBloom.height);
        ...
        private void genFlare(fx_Quad quad, fx_Special special)
        {
            ...
            OGL.Clear(ClearBufferMask.ColorBufferBit);
            OGL.Viewport(0, 0, _tFlare.width, _tFlare.height);
        ...
        private void blend(fx_Quad quad, FrameBuffer scene_fbo, Matrix4 camera_matrix)
        {
            ...
            OGL.Uniform(_pBlend.getUniform("lens_star_mod"), true, lens_star_mod);
