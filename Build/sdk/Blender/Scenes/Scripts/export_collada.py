...
export_as_scene = False

#######################################
## File Paths
#######################################

base_filename = bpy.path.basename(bpy.context.blend_data.filepath).replace(".blend","")
if export_as_scene:
    base_directory = '..\\Scene\\' + base_filename + '\\'
else:
    base_directory = '..\\Scene\\'
...
#######################################
	...
	for item in bpy.data.objects:
	   if(item.type == 'MESH'):
		   ...
		   # flags
                flags = "flg " + str(item.rigid_body.enabled) + " " + str(item.rigid_body.kinematic) + '\n'
                physics_file.write(flags)
		   # attributes
